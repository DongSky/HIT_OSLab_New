...
send_data(struct http_request *req, int fd)
{
	...
	int r;
	struct Stat stat;
	if((r = fstat(fd, &stat)) < 0)die("send_data failed: fstat failed");
	if(stat.st_size > 1518)die("send_data failed: size larger than 1518");
	buf = malloc(stat.st_size);
	if((r = readn(fd, buf, stat.st_size)) != stat.st_size)
		die("send_data failed: could not read all data");
	if ((r = write(req->sock, buf, stat.st_size)) != stat.st_size) {
		die("send_data failed: could not write all data in socket");
	}
	free(buf);
	return 0;
}
